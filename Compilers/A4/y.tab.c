...
#line 2 "yacc_parser.y"
	...
	int yylex();
	int yyerror();
	char *assignment_datatype;
	char var_names[256][128];
	int iterator;

#line 14 "yacc_parser.y"
#ifdef YYSTYPE
...
break;
case 29:
#line 91 "yacc_parser.y"
	{yyval.fValue = returnArithmeticOperationFloat(yystack.l_mark[-2].fValue, yystack.l_mark[-1].sValue, yystack.l_mark[0].fValue);}
break;
case 30:
#line 92 "yacc_parser.y"
	{

				char *dtype = returnDataType(&sTab, yystack.l_mark[0].sValue);

				if(strcmp(dtype, "int")==0 && strcmp(assignment_datatype, "int")==0){
					yyval.iValue = returnIntegerValue(&sTab, yystack.l_mark[0].sValue);
				}
				else if(strcmp(dtype, "int")==0 && strcmp(assignment_datatype, "float")==0){
					yyval.fValue = (float)returnIntegerValue(&sTab, yystack.l_mark[0].sValue);
				}
				else if(strcmp(dtype, "float")==0 && strcmp(assignment_datatype, "int")){
...
break;
case 31:
#line 110 "yacc_parser.y"
	{ 

				if(assignment_datatype == NULL){
					assignment_datatype = "int";
				}
				else if(strcmp(assignment_datatype, "float") == 0){
					assignment_datatype="float";
					yyval.fValue = (float)yystack.l_mark[0].iValue;
				}
				else if(strcmp(assignment_datatype, "int") == 0){
